...
export function navigationItems(input: { diagnosticCompleted: boolean; role?: NavigationRole }): NavigationItem[] {
  const items: NavigationItem[] = [
    ...
    { id: 'trilhas', label: 'Explorar trilhas', primary: true },
    { id: 'evolucao', label: 'Meu desenvolvimento', primary: true },
  ];
  if (!input.diagnosticCompleted) items.splice(1, 0, { id: 'diagnostico', label: 'Sondagem', primary: true });
  if (['superadmin', 'admin', 'rh'].includes(String(input.role))) items.push({ id: 'admin', label: 'Administração RH', primary: true });
  return items;
}
